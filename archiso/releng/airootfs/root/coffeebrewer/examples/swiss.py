...
mode only_os processes only the installation of Coffeelinux and software at --mountpoint (or /mnt/coffeebrewer)
...
import coffeebrewer
from coffeebrewer import ConfigurationOutput, NetworkConfigurationHandler, Menu
...
if coffeebrewer.arguments.get('help'):
	...
	exit(0)
if os.getuid() != 0:
	print("Coffeebrewer requires root privileges to run. See --help for more.")
	exit(1)
...
def select_mode():
	return coffeebrewer.generic_select(['full','only_hd','only_os','minimal','lineal'],
								'Select one execution mode',
								default=coffeebrewer.arguments.get('mode','full'))


"""
following functions will be at locale_helpers, so they will have to be called prefixed by coffeebrewer
"""
...
def reset_cmd_locale():
	...
	coffeebrewer.storage['CMD_LOCALE'] = coffeebrewer.storage.get('CMD_LOCALE_DEFAULT',{})

def unset_cmd_locale():
	""" coffeebrewer will use the execution environment default """
	coffeebrewer.storage['CMD_LOCALE'] = {}
...
def set_cmd_locale(general :str = None,
	...
	if general:
		if general in installed_locales:
			coffeebrewer.storage['CMD_LOCALE'] = {'LC_ALL':general}
		else:
			coffeebrewer.log(f"{get_locale_mode_text('LC_ALL')} {general} is not installed. Defaulting to C",fg="yellow",level=logging.WARNING)
		return
	...
	if numbers:
		if numbers in installed_locales:
			...
			coffeebrewer.log(f"{get_locale_mode_text('LC_NUMERIC')} {numbers} is not installed. Defaulting to installation language",fg="yellow",level=logging.WARNING)
	if charset:
		if charset in installed_locales:
			result["LC_CTYPE"] = charset
		else:
			coffeebrewer.log(f"{get_locale_mode_text('LC_CTYPE')} {charset} is not installed. Defaulting to installation language",fg="yellow",level=logging.WARNING)
	if time:
		if time in installed_locales:
			result["LC_TIME"] = time
		else:
			coffeebrewer.log(f"{get_locale_mode_text('LC_TIME')} {time} is not installed. Defaulting to installation language",fg="yellow",level=logging.WARNING)
	if collate:
		if collate in installed_locales:
			result["LC_COLLATE"] = collate
		else:
			coffeebrewer.log(f"{get_locale_mode_text('LC_COLLATE')} {collate} is not installed. Defaulting to installation language",fg="yellow",level=logging.WARNING)
	if messages:
		if messages in installed_locales:
			result["LC_MESSAGES"] = messages
		else:
			coffeebrewer.log(f"{get_locale_mode_text('LC_MESSAGES')} {messages} is not installed. Defaulting to installation language",fg="yellow",level=logging.WARNING)
	coffeebrewer.storage['CMD_LOCALE'] = result

def list_installed_locales() -> list[str]:
	lista = []
	for line in coffeebrewer.SysCommand('locale -a'):
...
def select_installed_locale(mode):
	...
	return coffeebrewer.generic_select([None] + list_installed_locales(),
								texto,
								allow_empty_input=True,
								default=coffeebrewer.storage.get('CMD_LOCALE',{}).get(mode,'C'))
...
class SetupMenu(coffeebrewer.AbstractMenu):
	def __init__(self,storage_area):
		super().__init__(data_store=storage_area)

	def _setup_selection_menu_options(self):
		self.set_option(
			'coffeebrewer-language',
			coffeebrewer.Selector(
				_('Coffeebrewer language'),
				lambda x: self._select_coffeebrewer_language(x),
	...
	def exit_callback(self):
		if self._data_store.get('ntp',False):
			coffeebrewer.log("Hardware time and other post-configuration steps might be required in order for NTP to work. For more information, please check the Coffee wiki.", fg="yellow")
			coffeebrewer.SysCommand('timedatectl set-ntp true')
		if self._data_store.get('mode',None):
			coffeebrewer.arguments['mode'] = self._data_store['mode']
			coffeebrewer.log(f"Coffeebrewer will execute under {coffeebrewer.arguments['mode']} mode")
		if self._data_store.get('LC_ALL',None):
			coffeebrewer.storage['CMD_LOCALE'] = {'LC_ALL':self._data_store['LC_ALL']}
		else:
			exec_locale = {}
			...
			coffeebrewer.storage['CMD_LOCALE'] = exec_locale
		coffeebrewer.log(f"Coffeebrewer will execute with {coffeebrewer.storage.get('CMD_LOCALE',None)} locale")

class MyMenu(coffeebrewer.GlobalMenu):
	def __init__(self,data_store=coffeebrewer.arguments,mode='full'):
	...
	def _setup_selection_menu_options(self):
		...
		if self._execution_mode in ('full','lineal'):
			...
			coffeebrewer.log(f"self._execution_mode {self._execution_mode} not supported")
			exit(1)
		if self._execution_mode != 'lineal':
			options_list.extend(['save_config','install','abort'])
			if not coffeebrewer.arguments.get('advanced'):
				options_list.append('coffeebrewer-language')
...
def ask_user_questions(mode):
	...
	"""
	if coffeebrewer.arguments.get('advanced',None):
		# 3.9 syntax. former x = {**y,**z} or x.update(y)
		set_cmd_locale(charset='es_ES.utf8',collate='es_ES.utf8')
		setup_area = coffeebrewer.storage.get('CMD_LOCALE',{}) | {}
		with SetupMenu(setup_area) as setup:
			if mode == 'lineal':
				...
				setup.run()
		coffeebrewer.arguments['coffeebrewer-language'] = setup_area.get('coffeebrewer-language')
	else:
		coffeebrewer.log("Hardware time and other post-configuration steps might be required in order for NTP to work. For more information, please check the Coffee wiki.", fg="yellow")
		coffeebrewer.SysCommand('timedatectl set-ntp true')

	with MyMenu(data_store=coffeebrewer.arguments,mode=mode) as global_menu:
		...
		if mode == 'lineal':
			for entry in global_menu.list_enabled_options():
				...
				coffeebrewer.arguments[entry] = global_menu.option(entry).get_selection()
		else:
...
def perform_filesystem_operations():
	...
	if coffeebrewer.arguments.get('harddrives', None):
		print(f" ! Formatting {coffeebrewer.arguments['harddrives']} in ", end='')
...
def os_setup(installation):
	...
	if installation.minimal_installation(
			hostname=coffeebrewer.arguments['hostname'],
			locales=[f"{coffeebrewer.arguments['sys-language']} {coffeebrewer.arguments['sys-encoding'].upper()}"]):
		if coffeebrewer.arguments['mirror-region'].get("mirrors", None) is not None:
			installation.set_mirrors(
				coffeebrewer.arguments['mirror-region'])  # Set the mirrors in the installation medium
		if coffeebrewer.arguments["bootloader"] == "grub-install" and coffeebrewer.has_uefi():
			installation.add_additional_packages("grub")
		installation.add_bootloader(coffeebrewer.arguments["bootloader"])
		if coffeebrewer.arguments['swap']:
		...
		if coffeebrewer.arguments['profile'] and coffeebrewer.arguments['profile'].has_post_install():
			with coffeebrewer.arguments['profile'].load_instructions(
				namespace=f"{coffeebrewer.arguments['profile'].namespace}.py") as imported:
...
if not coffeebrewer.check_mirror_reachable():
...
mode = coffeebrewer.arguments.get('mode', 'full').lower()
if not coffeebrewer.arguments.get('silent'):
...
perform_installation(coffeebrewer.storage.get('MOUNT_POINT', '/mnt'), mode)
