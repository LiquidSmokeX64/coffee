...
import coffeebrewer
from coffeebrewer import ConfigurationOutput


class OnlyHDMenu(coffeebrewer.GlobalMenu):
...
def ask_user_questions():
	...
	"""
	with OnlyHDMenu(data_store=coffeebrewer.arguments) as menu:
		# We select the execution language separated
		menu.exec_option('coffeebrewer-language')
		menu.option('coffeebrewer-language').set_enabled(False)
		menu.run()
...
def perform_installation(mountpoint):
	...
	"""
	with coffeebrewer.Installer(mountpoint, kernels=None) as installation:
		...
		for partition in installation.partitions:
			if partition.mountpoint == installation.target + '/boot':
				if partition.size <= 0.25: # in GB
					raise coffeebrewer.DiskError(f"The selected /boot partition in use is not large enough to properly install a boot loader. Please resize it to at least 256MB and re-run the installation.")
