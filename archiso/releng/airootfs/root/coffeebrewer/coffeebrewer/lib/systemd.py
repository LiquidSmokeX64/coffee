...
class Boot:
	def __init__(self, installation: Installer):
		...
		self.container_name = 'coffeebrewer'
		self.session = None
		self.ready = False

	def __enter__(self) -> 'Boot':
		if (existing_session := storage.get('active_boot', None)) and existing_session.instance != self.instance:
			raise KeyError("Coffeebrewer only supports booting up one instance, and a active session is already active and it is not this one.")
