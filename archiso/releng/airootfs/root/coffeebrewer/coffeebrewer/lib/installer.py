...
__packages__ = ["base", "base-devel", "linux-firmware", "linux", "linux-lts", "linux-zen", "linux-hardened", "yay"]
...
class Installer:
	...
	It also wraps :py:func:`~coffeebrewer.Installer.pacstrap` among other things.
	...
	:type partition: class:`coffeebrewer.Partition`
	...
	:param boot_partition: There's two reasons for needing a boot partition argument,
		...
		The second being when :py:func:`~coffeebrewer.Installer.add_bootloader` is called,
		A `boot_partition` must be known to the installer before this is called.
	:type boot_partition: class:`coffeebrewer.Partition`

	:param profile: A profile to install, this is optional and can be called later manually.
		This just simplifies the process by not having to call :py:func:`~coffeebrewer.Installer.install_profile` later on.
	...
	def __exit__(self, *args :str, **kwargs :str) -> None:
		...
		if len(args) >= 2 and args[1]:
			...
			print(_("    Please submit this issue (and file) to https://github.com/archlinux/coffeebrewer/issues"))
			raise args[1]
		...
		if not (missing_steps := self.post_install_check()):
			...
			self.log("Submit this zip file as an issue to https://github.com/archlinux/coffeebrewer/issues", level=logging.WARNING)
	...
	def pacstrap(self, *packages :str, **kwargs :str) -> bool:
		...
		try:
			...
			raise RequirementError("Pacstrap failed. See /var/log/coffeebrewer/install.log or above message for error details.")

    def yay(self, *packages :str, **kwargs :str) -> bool:
		if type(packages[0]) in (list, tuple):
			packages = packages[0]

		for plugin in plugins.values():
			if hasattr(plugin, 'on_yay'):
				if (result := plugin.on_yay(packages)):
					packages = result
		...
		# TODO: We technically only need to run the -Syy once.
		try:
			run_yay('--noconfirm --nodiffmenu --noremovemake --answerclean n  --answerdiff n --answeredit y --answerupgrade y', default_cmd='/usr/bin/yay')
		except SysCallError as error:
			self.log(f'Could not sync a new package database: {error}', level=logging.ERROR, fg="red")

			if storage['arguments'].get('silent', False) is False:
				if input('Would you like to re-try this download? (Y/n): ').lower().strip() in ('', 'y'):
					return self.yay(*packages, **kwargs)

			raise RequirementError(f'Could not sync mirrors: {error}', level=logging.ERROR, fg="red")

		try:
			return SysCommand(f'/usr/bin/yay -C /etc/pacman.conf {self.target} {" ".join(packages)} --noconfirm', peak_output=True).exit_code == 0
		except SysCallError as error:
			self.log(f'Could not strap in packages: {error}', level=logging.ERROR, fg="red")
			...
			raise RequirementError("yay failed. See /var/log/coffeebrewer/install.log or above message for error details.")
	...
	def minimal_installation(
			...
			hostname: str = 'coffeebrewer', locales: List[str] = ['en_US.UTF-8 UTF-8']) -> bool:
		...
		if not is_vm():
			...
			if vendor == "AuthenticAMD":
				...
				self.log(f"Unknown CPU vendor '{vendor}' detected. Coffeebrewer won't install any ucode.", level=logging.DEBUG)
		...
		#
		# https://github.com/archlinux/coffeebrewer/issues/880
	...
	def setup_swap(self, kind :str = 'zram') -> bool:
		if kind == 'zram':
			...
			# We could use the default example below, but maybe not the best idea: https://github.com/archlinux/coffeebrewer/pull/678#issuecomment-962124813
			...
			raise ValueError(f"Coffeebrewer currently only supports setting up swap on zram")
	...
	def add_systemd_bootloader(self, boot_partition :Partition, root_partition :Partition) -> bool:
		...
		for kernel in self.kernels:
			...
			with open(f'{self.target}/boot/loader/entries/{self.init_time}_{kernel}.conf', 'w') as entry:
				entry.write('# Created by: coffeebrewer\n')
				...
				if not is_vm():
					...
					if vendor == "AuthenticAMD":
						...
						self.log(f"Unknown CPU vendor '{vendor}' detected. Coffeebrewer won't add any ucode to systemd-boot config.", level=logging.DEBUG)
				...
				#
				# https://github.com/archlinux/coffeebrewer/issues/881
				...
				if real_device := self.detect_encryption(root_partition):
					...
					if self._disk_encryption.hsm_device:
						...
						# https://github.com/archlinux/coffeebrewer/pull/1196#issuecomment-1129715645
		...
		for kernel in self.kernels:
			...
			if not is_vm():
				...
				if vendor == "AuthenticAMD":
					...
					self.log(f"Unknown CPU vendor '{vendor}' detected. Coffeebrewer won't add any ucode to firmware boot entry.", level=logging.DEBUG)
	...
	def add_bootloader(self, bootloader :str = 'systemd-bootctl') -> bool:
		...
		Coffeebrewer supports one of three types:
		* systemd-bootctl
		* grub
		* efistub (beta)
...
#
            ...
            printf "%b1\n" "$*" | arch-chroot /mnt yay --noconfirm --nodiffmenu --noremovemake --answerclean a  --answerdiff n --answeredit y --answerupgrade y shim-signed
            printf "%b1\n" "$*" | arch-chroot /mnt yay --noconfirm --nodiffmenu --noremovemake --answerclean a  --answerdiff n --answeredit y --answerupgrade y preloader-signed
            ...
            arch-chroot /mnt efibootmgr --unicode --disk "$boot_partition" --part 1 --create --label "PreLoader" --loader /EFI/systemd/PreLoader.efi
            ...
            arch-chroot /mnt efibootmgr --unicode --disk "$boot_partition" --part 1 --create --label "Shim" --loader /EFI/BOOT/BOOTx64.EFI
            ...
            printf "%b\nNot installing secure-boot on your machine\n" "$*"
...
#
    ...
    def add_additional_packages_yay(self, *packages :str) -> bool:
		return self.yay(*packages)

	def install_profile(self, profile :str) -> ModuleType:
		"""
		Installs a coffeebrewer profile script (.py file).
		...
		self.log(f'Installing coffeebrewer profile {profile}', level=logging.INFO)
	...
	def set_keyboard_language(self, language: str) -> bool:
		...
		if len(language.strip()):
			...
			# In accordance with https://github.com/archlinux/coffeebrewer/issues/107#issuecomment-841701968
			# Setting an empty keymap first, allows the subsequent call to set layout for both console and x11.
			from .systemd import Boot
			with Boot(self) as session:
				os.system('/usr/bin/systemd-run --machine=coffeebrewer --pty localectl set-keymap ""')
