...
# 1: List coffeebrewer.plugin definitions
# 2: Load the plugin entrypoint
# 3: Initiate the plugin and store it as .name in plugins
for plugin_definition in metadata.entry_points().select(group='coffeebrewer.plugin'):
...
def load_plugin(path :str) -> ModuleType:
	...
	if namespace in sys.modules:
		# Version dependency via __coffeebrewer__version__ variable (if present) in the plugin
		# Any errors in version inconsistency will be handled through normal error handling if not defined.
		if hasattr(sys.modules[namespace], '__coffeebrewer__version__'):
			coffeebrewer_major_and_minor_version = float(storage['__version__'][:find_nth(storage['__version__'], '.', 2)])

			if sys.modules[namespace].__coffeebrewer__version__ < coffeebrewer_major_and_minor_version:
				log(f"Plugin {sys.modules[namespace]} does not support the current Coffeebrewer version.", fg="red", level=logging.ERROR)
