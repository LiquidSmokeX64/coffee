...
class GlobalMenu(AbstractMenu):
	...
	def _setup_selection_menu_options(self):
		# coffeebrewer.Language will not use preset values
		self._menu_options['coffeebrewer-language'] = \
			Selector(
		...
		if harddrives is not None:
			if len(harddrives) == 0:
				prompt = _(
					...
					"WARNING: Coffeebrewer won't check the suitability of this setup\n"
	...
	def _select_profile(self, preset):
		...
		if profile is None:
			if any([
				coffeebrewer.storage.get('profile_minimal', False),
				coffeebrewer.storage.get('_selected_servers', None),
				coffeebrewer.storage.get('_desktop_profile', None),
				coffeebrewer.arguments.get('desktop-environment', None),
				coffeebrewer.arguments.get('gfx_driver_packages', None)
			]):
				return preset
			else:  # ctrl+c was actioned and all profile settings have been reset
				return None

		servers = coffeebrewer.storage.get('_selected_servers', [])
		desktop = coffeebrewer.storage.get('_desktop_profile', None)
		desktop_env = coffeebrewer.arguments.get('desktop-environment', None)
		gfx_driver = coffeebrewer.arguments.get('gfx_driver_packages', None)
